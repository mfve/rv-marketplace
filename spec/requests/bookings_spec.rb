  ...
  describe "GET /api/bookings" do
    context "happy path" do
      ...
      it "returns bookings for the current user" do
        get "/api/bookings", headers: headers1
  ...
  describe "POST /api/bookings" do
    ...
    context "happy path" do
      it "creates a new booking" do
        expect {
          post "/api/bookings", params: valid_params, headers: headers1
    ...
    context "sad path" do
      it "returns error when booking own listing" do
        ...
        post "/api/bookings", params: own_params, headers: headers1
      ...
      it "returns error for invalid date format" do
        ...
        post "/api/bookings", params: invalid_params, headers: headers1
        expect(response).to have_http_status(:unprocessable_entity)
        json = JSON.parse(response.body)
  ...
  describe "POST /api/bookings/:id/confirm" do
    ...
    context "happy path" do
      it "confirms the booking" do
        post "/api/bookings/#{booking.id}/confirm", headers: headers2
        expect(response).to have_http_status(:ok)
    ...
    context "sad path" do
      it "returns not_found for non-owner" do
        post "/api/bookings/#{booking.id}/confirm", headers: headers1
        expect(response).to have_http_status(:not_found)
      end
    end
  end

  describe "POST /api/bookings/:id/reject" do
    let!(:booking) { create(:booking, user: user1, rv_listing: rv_listing, status: "pending") }

    context "happy path" do
      it "rejects the booking" do
        post "/api/bookings/#{booking.id}/reject", headers: headers2
        expect(response).to have_http_status(:ok)
